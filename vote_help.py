...
id = input("请输入帮投id：")
delay = int(input("请输入帮投间隔(推荐29秒)："))
auto = input("是否需要自动获取代理(y/n)：")
votenum = int(input("需要帮忙投几票："))
    ...
    try:
        ...
        if (code == "1"):
            try:
                ...
                vote_count += 1
                print("%s帮投成功！一共投了%s票，当前票数为%s，距上次投票为%s秒，%s秒后再投" % (id,vote_count,piaoshu,(datetime.datetime.now() - vote_time).seconds,delay))
                ...
                vote_count += 1
                print("%s帮投成功！一共投了%s票，但无法读取当前票数，距上次投票为%s秒，%s秒后再投" % (id, vote_count, (datetime.datetime.now() - vote_time).seconds, delay))
                print("程序启动时间为%s,当前时间为%s" %(starttime.strftime('%Y-%m-%d %H:%M:%S'),datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
                vote_time = datetime.datetime.now()
                if (vote_count>=votenum):
                    print("帮投任务完成!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            ...
            try:
                ...
                print("帮投失败！%s" % str.encode('utf-8').decode('unicode_escape'))
                continue
            except:
                print("帮投失败！")
                continue